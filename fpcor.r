...
reformat_fp = function(tf, fp, include_patientID, controls, dtype) {
  ...
  if (!is.null(include_patientID)) {
    check_valid_sample(include_patientID, tf, "source")
...
corr_fp = function(source_fp, source_tf, reference_fp, 
  ...
  files = lapply(c(source_tf, reference_tf, source_fp, reference_fp), function(i){
    read.csv(i, sep="\t", header = T, colClasses = "character")})
  
  source_fp = reformat_fp(files[[1]], files[[3]], 
			  source_patientID, exclude_controls, "source")

  reference_fp = reformat_fp(files[[2]], files[[4]],
			     reference_patientID, exclude_controls, "reference")
...
parser$add_argument('-rt', '--reference_title_file', required = TRUE,
                    help = 'Title file of the reference project')
parser$add_argument('-rp', '--reference_FP_file', required = TRUE,
